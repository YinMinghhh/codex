...
- `thread/tokenUsage/updated` — `{ threadId, turnId, tokenUsage }` where `tokenUsage.contextWindowBreakdown` is `null` for older/replayed usage or contains the latest model request context composition. Breakdown components include taxonomy category, source label, JSON pointer into the final OpenAI request, optional request text range, optional assembly step ids, optional mapping confidence, estimated bytes/tokens, content hash, and the full `value` slice from the request. Treat this field as sensitive local debug data: it can contain complete prompts, local paths, tool output, API responses, and other request fragments.
